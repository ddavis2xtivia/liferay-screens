...
extension BaseScreenlet {
		...
		internal func allowCloseOnTouch() -> Bool {
			...
			switch self {
				case .AutocloseComputedDelay(let touchClose):
					result = touchClose
				case .AutocloseDelayed( _, let touchClose):
					result = touchClose
	...
	*/
		...
		synchronized(BaseScreenletHudLock) {
			...
			if var delay = closeDelay {
				if delay == Double.infinity {
					...
					let len: Int =
    	                MBProgressHUDInstance.instance!.labelText!.characters.count +
						MBProgressHUDInstance.instance!.detailsLabelText!.characters.count
	...
	public func showHUDAlert(message message: String, details: String? = nil) {
